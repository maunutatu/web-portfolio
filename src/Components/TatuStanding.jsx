...
import React, { useEffect, useRef, forwardRef, useImperativeHandle, useReducer, useState } from 'react'
import { useGLTF, useAnimations } from '@react-three/drei'

// eslint-disable-next-line react/display-name
const TatuAnimations = forwardRef((props, ref) => {
  const group = useRef()
  const { nodes, materials } = useGLTF('./models/TatuStanding.gltf')

  const standAnimationClip = useGLTF('./models/TatuStanding.gltf').animations[0]
  const waveAnimationClip = useGLTF('./models/TatuWaving.gltf').animations[0]
  const textAnimationClip = useGLTF('./models/TatuTexting.gltf').animations[0]
  const kickAnimationClip = useGLTF('./models/TatuKicking.gltf').animations[0]

  const animations = [{ ...standAnimationClip, name: 'stand' }, { ...waveAnimationClip, name: 'wave' }, { ...textAnimationClip, name: 'text' }, { ...kickAnimationClip, name: 'kick' }]
  ...
  useImperativeHandle(ref, () => {
    return {
      currentAnimationRef,
      mixer,
      waveAnimation: actions.wave,
      textAnimation: actions.text,
